    ...
    def key(event):
        ...
        if s:
            ...
            while 1:
                ...
                text.tag_add('foundin', idx, lastidx)
                idx = lastidx
            text.tag_config('foundin', foreground="red")
        d = "class "
        if d:
            idx = '1.0'
            while 1:
                idx = text.search(d, idx, nocase=1, stopindex=END)
                if not idx: break
                lastidx = '%s+%dc' % (idx, len(d))
        ...
        if z:
            idx = '1.0'
            while 1:
                idx = text.search(z, idx, nocase=1, stopindex=END)
                if not idx: break
                lastidx = '%s+%dc' % (idx, len(z))
                text.tag_add('foundfun', idx, lastidx)
                idx = lastidx
            text.tag_config('foundfun', foreground="blue")
        f = "while "
        if f:
            idx = '1.0'
            while 1:
                idx = text.search(f, idx, nocase=1, stopindex=END)
                if not idx: break
                lastidx = '%s+%dc' % (idx, len(f))
                text.tag_add('foundfun', idx, lastidx)
                idx = lastidx
            text.tag_config('foundfun', foreground="blue")
        y = "if "
        if y:
            idx = '1.0'
            while 1:
                idx = text.search(y, idx, nocase=1, stopindex=END)
                if not idx: break
                lastidx = '%s+%dc' % (idx, len(y))
                text.tag_add('foundfun', idx, lastidx)
                idx = lastidx
            text.tag_config('foundfun', foreground="blue")
        bi = "#ape"
        if bi:
            idx = '1.0'
            while 1:
                idx = text.search(bi, idx, nocase=1, stopindex=END)
                if not idx: break
                lastidx = '%s+%dc' % (idx, len(bi))
                text.tag_add('yellow', idx, lastidx)
                idx = lastidx
            text.tag_config('yellow', foreground="brown")
        tri = "#max-memory"
        if tri:
            idx = '1.0'
            while 1:
                idx = text.search(tri, idx, nocase=1, stopindex=END)
                if not idx: break
                lastidx = '%s+%dc' % (idx, len(tri))
                text.tag_add('yellow', idx, lastidx)
                idx = lastidx
            text.tag_config('yellow', foreground="brown")
        tri = ")"
